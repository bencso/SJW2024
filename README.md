# Skills Junior 2024 🏆💻

> Webfejlesztő feladatok - Skills Junior 2024 verseny 2. forduló

## 🎯 Projekt célja

Ez a repository a [**Skills Junior 2024**](https://www.linkedin.com/posts/http-alap%C3%ADtv%C3%A1ny_skillsit-juniorskills2024-httpalapitvany-activity-7183126229829439489-pGPE/) versenysorozat webfejlesztő kategóriájának **2. fordulójához** készült. A feladat gyakorlati webfejlesztési kihívások megoldása volt időkorlátos versenyhelyzetben.

## 🏅 Eredmény

**Országos 12. helyezett** - A közel 500 jelentkezőből 12. helyezett lettem

## 🛠️ Technológiák

- **HTML5**
- **CSS3** - Modern styling és layout
- **JavaScript** - Interaktivitás és logika

## 💭 Fejlesztési folyamat

A verseny során időnyomás alatt kellett webfejlesztési standardok szerint dolgozni. A feladatok valós projektfeladatokat szimuláltak, pontos specifikációk alapján kellett megoldani őket.

## 🎓 Tanulási célok

A projekt során gyakoroltam:
- Webfejlesztés versenyhelyzetben
- Időgazdálkodás és hatékonyság
- Precíz munkavégzés specifikációk alapján

## 📚 A versenyről

### Mi a Skills Junior?

> A **Skills Junior** egy rangos nemzeti versenysorozat, amely a **WorldSkills Hungary** program része. A verseny 18 év alatti középiskolások számára szervezett, ahol a tehetséges fiatal diákok mérik össze tudásukat.

### Verseny menete

1. **Online selejtező** - Elméleti és alapvető feladatok
2. **Középdöntő (2. forduló)** - Gyakorlati kódolási feladatok
3. **Országos döntő** - Élő verseny

## 🤝 Közreműködés

Ez egy lezárt verseny projekt, amely a SkillsIt Junior 2024 középdöntőjére készült.

***

**Státusz:** ✅ Befejezett

---

**⭐ Ha tetszik a munka, örülök egy csillagnak!**

---
